...
// sales-sync.js
import { supabase } from "./supabaseClient.js";
import { resolveProductIdsForSquareIds } from "./syncToSupabase.js"; // <-- NEW IMPORT
...
async function fetchVariationsMap() {
  ...
  console.log(
    `[fetchVariationsMap] mapped ${Object.keys(map).length} variations`
  );
  return map;
}
...
// REMOVED fetchProductsMap() & fetchVariationsMap()
// They are replaced by the single call to resolveProductIdsForSquareIds()
...
async function fetchLocationsMap() {
  ...
  console.log(
    `[fetchLocationsMap] mapped ${Object.keys(map).length} locations`
  );
  return map;
}
...
// Fetch recent orders from Square and flatten to line-item rows
// ---------------------------------------------------------------
async function fetchRecentSalesFromSquare(
  options,
  squareLocationIds = [],
  days = 14
) {
  if (!Array.isArray(squareLocationIds) || squareLocationIds.length === 0) {
    throw new Error(
      "fetchRecentSalesFromSquare requires an array of location IDs"
    );
  }
  ...
  for (let i = 0; i < squareLocationIds.length; i += BATCH_LOCATIONS) {
    ...
    do {
      ...
      if (orders.length) {
        console.log(
          `[fetchRecentSalesFromSquare] fetched ${orders.length} orders, page ${
            page + 1
          })`
        );
        for (const order of orders) {
          ...
          for (const li of items) {
            ...
            const qty = li.quantity ? Number(li.quantity) : 1;
            const totalCents =
              li.total_money && typeof li.total_money.amount === "number"
            ...
            lineItems.push({
              ...
              sale_date: closedAt,
              total_money_cents:
                typeof totalCents === "number" ? totalCents : null,
              __raw_line_item: li,
            });
          }
        }
        ...
        console.log(
          `[fetchRecentSalesFromSquare] no orders on page ${
            page + 1
          } for locations ${locs.join(", ")}`
        );
      }
  ...
  console.log(
    `[fetchRecentSalesFromSquare] total line items fetched: ${lineItems.length}`
  );
  return lineItems;
}
...
// async function fetchAndMapRecentSales(
//   options,
//   squareLocationIds = [],
//   opts = { skipUnmapped: true }
// ) {
//   const lineItems = await fetchRecentSalesFromSquare(
//     options,
//     squareLocationIds
//   );

//   // Get all unique catalog IDs from the fetched line items
//   const allCatalogIds = [
//     ...new Set(lineItems.map((li) => li.catalog_object_id).filter(Boolean)),
//   ];

//   // Get the maps for products, variations, and locations
//   const [productMap, variationMap, locationMap] = await Promise.all([
//     fetchProductsMap(), // You need these functions again or a new resolver
//     fetchVariationsMap(), // You need these functions again or a new resolver
//     fetchLocationsMap(),
//   ]);
//   // Note: The above is a quick fix. A better solution would be to use a
//   // single resolver function that returns both product and variation UUIDs.
...
//     if (catalogId && variationMap[catalogId]) {
//       // Found a variation mapping
//       variation_id = variationMap[catalogId].id;
//       product_id = variationMap[catalogId].product_id;
//     } else if (catalogId && productMap[catalogId]) {
//       // Found a product mapping (no variation)
//       product_id = productMap[catalogId];
//       variation_id = null;
//     } else {
//       // No mapping found for the catalog ID
//       if (opts.skipUnmapped) {
//         skipped.push({ catalogId, orderId, lineUid });
//         continue;
//       } else {
...
//     const qty = Number(li.quantity || 0);
//     const cents =
//       typeof li.total_money_cents === "number" ? li.total_money_cents : null;
//     const total_amount = cents !== null ? cents / 100 : null;
//     const unit_price = cents !== null && qty > 0 ? cents / qty / 100 : null;
...
//       square_updated_at: null,
//       synced_at: new Date().toISOString(),
//       variation_id, // <-- USE THE CORRECT, MAPPED UUID HERE
//       unit_price,
//       total_amount,
//       order_id: orderId,
//     });
...
//   if (skipped.length) {
//     console.warn(
//       `[fetchAndMapRecentSales] skipped ${skipped.length} line-items with unmapped catalog IDs (example):`,
//       skipped.slice(0, 5)
//     );
//   }

//   console.log(
//     `[fetchAndMapRecentSales] mapped ${rows.length} sales rows (skipped ${skipped.length})`
//   );
//   return rows;
// }
...
async function fetchAndMapRecentSales(
  ...
  // 1. Get all unique catalog IDs from the fetched line items
  const allCatalogIds = [
    ...new Set(lineItems.map((li) => li.catalog_object_id).filter(Boolean)),
  ];

  // 2. Use the new resolver function to get the UUID mappings
  const mappedIds = await resolveProductIdsForSquareIds(allCatalogIds);

  const [locationMap] = await Promise.all([fetchLocationsMap()]);
  ...
  for (const li of lineItems) {
    const orderId = li.order_id;
    const lineUid = li.line_item_uid || "noluid";
    const squareSaleId = `${orderId}::${lineUid}`;
    ...
    // 3. Use the new mappedIds object to resolve the UUIDs
    const resolvedIds = mappedIds[catalogId];

    if (resolvedIds) {
      variation_id = resolvedIds.variation_id;
...
async function upsertSales(rows = []) {
  ...
  for (let i = 0; i < rows.length; i += SALES_UPSERT_BATCH) {
    ...
    if (error) {
      console.error(
        `upsertSales: error on batch ${Math.floor(i / SALES_UPSERT_BATCH) + 1}`,
        error
      );
      throw error;
    } else {
      console.log(
        `upsertSales: upserted batch ${
          Math.floor(i / SALES_UPSERT_BATCH) + 1
        } (${batch.length} rows)`
      );
    }
  }
...
// Top-level sync function wiring
// ---------------------------------------------------------------
export async function salesSync({
  skipUnmapped = true,
  lookbackDays = 14,
} = {}) {
  console.log("Starting salesSync...");
  ...
  if (!locRes.ok) {
    const text = await locRes.text().catch(() => "");
    throw new Error(
      `Failed to fetch Square locations: ${locRes.status} - ${text}`
    );
  }
  const locData = await locRes.json();
  const squareLocationIds = (locData.locations || [])
    .map((l) => l.id)
    .filter(Boolean);
  ...
  const mappedRows = await fetchAndMapRecentSales(options, squareLocationIds, {
    skipUnmapped,
  });

  if (mappedRows.length === 0) {
    console.log(
      "salesSync: no mapped rows to upsert (all rows may have been skipped)."
    );
    return;
  }
