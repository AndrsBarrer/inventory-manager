...
//           throw new Error(
//             `Failed to fetch inventory batch: ${res.status} - ${text}`
//           );
//         }
//         const data = await res.json();
//         if (data.counts) results.push(...data.counts);
//         console.log(
//           `Fetched inventory batch ${currentIndex + 1}/${chunks.length}`
//         );
//       } catch (err) {
...
//   console.log(
//     `[fetchMappingsInBatches] Resolving ${squareIds.length} squareIds in chunks of ${chunkSize}`
//   );
...
//       if (error) {
//         console.error(
//           `[fetchMappingsInBatches] Error on batch ${i / chunkSize + 1}:`,
//           {
...
//       }
//       console.log(
//         `[fetchMappingsInBatches] Batch ${i / chunkSize + 1} succeeded (${
//           chunk.length
//         } ids)`
//       );
...
//     } catch (err) {
//       console.error(
//         `[fetchMappingsInBatches] Unexpected failure on batch ${
//           i / chunkSize + 1
//         }:`,
//         err
//       );
...
//   console.log(
//     `[fetchProductsInBatchesWithSquareIds] Resolving ${squareIds.length} product ids in chunks of ${chunkSize}`
//   );
...
//       if (error) {
//         console.error(
//           `[fetchProductsInBatchesWithSquareIds] Error on batch ${
//             i / chunkSize + 1
//           }:`,
//           {
//             chunkSize: chunk.length,
...
//       }
//       console.log(
//         `[fetchProductsInBatchesWithSquareIds] Batch ${
//           i / chunkSize + 1
//         } succeeded (${chunk.length} ids)`
//       );
//       results.push(...data);
//     } catch (err) {
//       console.error(
//         `[fetchProductsInBatchesWithSquareIds] Unexpected failure on batch ${
//           i / chunkSize + 1
//         }:`,
//         err
//       );
...
// async function resolveProductIdsForSquareIds(
//   squareIds,
//   { allowCreateFallbackProducts = false } = {}
// ) {
...
//     console.log(
//       `[] Found ${directProducts.length} direct products in products table`
//     );
//   } catch (err) {
...
//     try {
//       console.log(
//         `[] Looking up ${unresolved.length} ids in product_variations (variation -> parent product)`
//       );
//       const chunkSize = 200;
...
//       }
//       console.log(
//         `[] ${unresolved.length} left unresolved after checking product_variations`
//       );
//     } catch (err) {
...
//       console.warn(
//         `[] ${unresolved.length} ids are unresolved and will NOT be auto-created as products (allowCreateFallbackProducts=false).`
//       );
...
//       console.log(
//         `[] Inserting ${newProducts.length} fallback products (opt-in)`
//       );
...
//         // attempt retry fetch
//         try {
//           const retryProducts = await fetchProductsInBatchesWithSquareIds(
//             unresolved
//           );
...
//     ...new Set(counts.map((c) => c.catalog_object_id).filter(Boolean)),
//   ];
//   const locationSquareIds = [
//     ...new Set(counts.map((c) => c.location_id).filter(Boolean)),
//   ];
...
//     .in(
//       "square_id",
//       locationSquareIds.length ? locationSquareIds : ["__none__"]
//     );
...
//     return;
//   }
//   const foundLocationMap = new Map(
//     (foundLocations || []).map((l) => [l.square_id, l.id])
//   );
...
//       console.warn(
//         `Missing mapping for variation ${val.catalog_object_id} or location ${val.location_id}; skipping.`
//       );
//       continue;
//     }
...
//       quantity,
//       square_updated_at: val.calculated_at
//         ? new Date(val.calculated_at).toISOString()
//         : null,
//     });
//   }
...
//   const baseUrl =
//     "https://connect.squareup.com/v2/catalog/list?types=ITEM,ITEM_VARIATION,CATEGORY";
//   const options = {
//     headers: {
...
//     let categoryName = itemData.category_id
//       ? categoryNameById[itemData.category_id]
//       : null;
...
//           console.log(
//             `Found product via variation mapping: ${item.id} -> ${productId} (${existingProduct?.name})`
//           );
//         }
//       }
...
//       if (!existingProduct) {
//         const nameMatches = existingProducts.filter(
//           (p) => p.name === itemData.name
//         );
...
//             return (current.synced_at || "") > (best.synced_at || "")
//               ? current
//               : best;
//           }, null);
//           console.log(
//             `Multiple products found for "${itemData.name}", selected: ${existingProduct?.square_id} (category: ${existingProduct?.category})`
//           );
//         }
//       }
...
//         console.log(
//           `Preserving category "${categoryName}" for "${itemData.name}" (Square ID: ${item.id})`
//         );
//       }
//     }
...
//       square_updated_at: item.updated_at
...
//     console.log(
//       `Upserting batch ${Math.floor(i / batchSize) + 1} of products.`
//     );
...
//     if (error) {
//       console.error(
//         `Error upserting products batch ${Math.floor(i / batchSize) + 1}:`,
//         error
//       );
//       throw error;
//     }
...
//       return parentItemId && productIdMap[parentItemId];
//     })
//     .map((variation) => {
...
//           ? (variationData.price_money.amount / 100).toString()
//           : null,
...
//   // Upsert the variations into the product_variations table
//   const batchSize = 100;
//   for (let i = 0; i < variationsToUpsert.length; i += batchSize) {
//     const batch = variationsToUpsert.slice(i, i + batchSize);
//     const { error } = await supabase.from("product_variations").upsert(batch, {
...
//     if (error) {
//       console.error(
//         `Error upserting variations batch ${Math.floor(i / batchSize) + 1}:`,
//         error
//       );
//       throw error;
//     }
...
//   const mappingRows = variationsToUpsert.map((v) => ({
//     square_id: v.square_variation_id, // This is the ID from Square's catalog object
//     product_id: v.product_id, // This links to the parent product in your database
//     variation_id: v.square_variation_id, // This links to the variation itself
//   }));

//   if (mappingRows.length > 0) {
//     console.log(
//       `Upserting ${mappingRows.length} mapping rows for variations...`
//     );
...
//     }
//     console.log(
//       `Successfully upserted ${mappingRows.length} variation mappings.`
//     );
//   }
// }
...
//   );
//   if (!locRes.ok)
//     throw new Error(`Failed to fetch locations: ${locRes.status}`);
...
//   const baseUrl =
//     "https://connect.squareup.com/v2/catalog/list?types=ITEM_VARIATION";
//   const options = {
//     headers: {
//       "Square-Version": "2023-08-16",
//       Authorization: `Bearer ${process.env.SQUARE_ACCESS_TOKEN}`,
...
async function upsertLocations(locations) {
  ...
  const { error } = await supabase.from("locations").upsert(toUpsert, {
    onConflict: "square_id",
  });
  if (error) console.error("Error upserting locations:", error);
  else console.log(`Upserted ${toUpsert.length} location(s)`);
