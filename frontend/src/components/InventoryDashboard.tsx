...
export const InventoryDashboard: React.FC = () => {
  ...
  const [syncType, setSyncType] = useState<
    "full" | "products" | "locations" | "sales" | "inventory"
  >("full");
  ...
  const fetchLowStock = async () => {
    ...
    try {
      ...
      const apiLocations = (data.locations || []).filter(
        (loc: any) =>
          loc.name === "Wolf Liquor" ||
          loc.name === "MAIN ST MARKET" ||
          loc.name === "Surf Liquor"
      );
      console.log(data);

      // flatten and normalize product shape
      const allLowStockProducts = apiLocations
        .flatMap((loc: any) =>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Low-stock items are based on 2-week average sales and configured
            reorder rules.
          </p>
        </CardContent>
      </Card>
              ...
              {currentItems.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {currentItems.map((item) => {
                    ...
                    const casesToOrder = unitsPerCase
                      ? Math.ceil(suggestedUnits / unitsPerCase)
                      : null;
                    const currentStock =
                      item.inventory === null || item.inventory === undefined
                        ...
                        : item.inventory;
                    const lowStockFlag =
                      item.low_stock !== undefined && item.low_stock !== null
                        ? Boolean(item.low_stock)
                        : (item.inventory ?? 0) <
                          (item.minimumStock ?? Infinity);
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Avg daily sales (14d): {fmtAvg(item.salesPerDay)}
                          </p>
                          {item.unitsPerCase !== undefined && (
                            <p className="text-xs text-muted-foreground">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          {unitsPerCase ? (
                            <div className="text-xs text-muted-foreground">
                              Order in cases of {unitsPerCase}
                            </div>
                          ) : (
                            <div className="text-xs text-muted-foreground">
                              Order in units (case size unknown)
                            </div>
                          )}
                        </div>
                      </li>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div
            className={`fixed inset-0 bg-black transition-opacity duration-300 ${
              modalAnimate ? "opacity-50" : "opacity-0"
            }`}
          ...
          >
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">
                This will pull the latest data from Square into the database and
                update the dashboard with the most recent information.
              </p>
